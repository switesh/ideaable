...
import revenueModels from '$lib/data/revenue-models.json'
import X from '$lib/components/icons/x.svelte'
let title = 'Revenue models'
</script>
...
<section class=" ">
  <div class="py-4 px-16 flex gap-4 justify-between items-center">
    ...
    <div class="flex gap-x-4">
      <input type="search" placeholder="Search revenue models">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 bg-slate-100 p-16 mt-8">
    {#each revenueModels as { title, description, image, caseStudy, examples }, index}
      <button aria-posinset={index + 1} aria-setsize={revenueModels.length} class="p-8 bg-white shadow-sm rounded flex flex-col gap-4 text-left" on:click={()=>revenueModelDetailsDialog.showModal()}>
...
<dialog id="revenueModelDetailsDialog" class="absolute top-0 right-0 max-w-screen-lg h-full">
  <div class="px-6 py-3 flex justify-between bg-slate-50 sticky top-0">
    <h1>Business model</h1>
    <div class="flex gap-x-4">
      <button on:click={()=>revenueModelDetailsDialog.close()}><X/></button>
    </div>
  </div>
